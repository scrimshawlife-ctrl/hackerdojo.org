...
import { formatMoney, parseMoney, roundMoney } from './money.js';
...
export async function closeArtwork(id, admin) {
  ...
  if (!result.alreadyClosed) {
    ...
    const title = result.artwork.title;
    ...
    if (result.winner?.email) {
      try {
        await sendAuctionEmail({
          ...
          text: [
            `Hi ${result.winner.name || 'there'},`,
            '',
            `Congratulations — you won "${title}" with a bid of $${formatMoney(result.winner.amount)}.`,
            'Hacker Dojo will contact you about payment and pickup.',
            '',
            url,
          ].join('\n'),
          meta: { winning_amount: formatMoney(result.winner.amount) },
        });
      } catch (err) {
    ...
    // Notify this admin (and any other admins lightly — MVP: acting admin only + log)
    try {
      await sendAuctionEmail({
        userId: admin.id,
        type: 'auction_closed',
        ...
        text: [
          `Lot closed: "${title}"`,
          result.winner
            ? `Winner: ${result.winner.email} at $${formatMoney(result.winner.amount)}`
            : 'Winner: none (no bids)',
          '',
          url,
        ].join('\n'),
        meta: {
          winner_email: result.winner?.email || null,
          winning_amount: result.winner ? formatMoney(result.winner.amount) : null,
        },
      });
    } catch (err) {
